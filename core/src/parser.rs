    ...
    I: Input,
{
    ...
    type Error: ParseError<I::Position>;
    ...
    E: ParseError<I::Position>,
{
    move |mut input: I| {
        if let Some(next) = input.iter().next() {
            ...
            Err(ControlFlow::Recovable(
                ErrorKind::Next(input.to_span()).into(),
            ))
        } else {
    ...
    F: FnOnce(I::Item) -> bool,
{
    move |mut input: I| {
        if let Some(next) = input.iter().next() {
            ...
            Err(ControlFlow::Recovable(
                ErrorKind::NextIf(input.to_span()).into(),
            ))
        } else {
            Err(ControlFlow::Incomplete(
                ErrorKind::NextIf(input.to_span()).into(),
    ...
    KW: Debug + Clone,
{
    move |mut input: I| {
        if let Some(len) = input.starts_with(keyword.clone()) {
            ...
            Err(ControlFlow::Recovable(
                ErrorKind::Keyword(input.to_span()).into(),
            ))
        }
    }
}
    ...
    E: ParseError<I::Position>,
{
    move |mut input: I| {
        ...
        return Err(ControlFlow::Incomplete(
            ErrorKind::TakeUntil(input.to_span()).into(),
        ));
    }
}
